...
1. Complete the T1 exercise in the editor
2. Create your own WPL and OML in your workspace
3. Configure Source and Sink
4. Use `wpgen` to generate sample data, then run `wparse` to view the output
...
- **data/**: Storage location for all data files
  ...
  - **out_dat/**: Store parsed output data
    ...
    - `default.dat`: Default output file
    - `error.dat`: Records that encountered errors during parsing
    - `ignore.dat`: Records ignored by rules
    - `miss.dat`: Records that did not match WPL rules
...
- **models/**: Core business logic (WPL rules and OML rules)
  ...
  - **knowledge/**: Knowledge base such as SQL queries and IP databases
